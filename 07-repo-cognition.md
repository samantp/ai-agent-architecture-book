...
- Codex system prompt: *"When searching for text or files, prefer using `rg` or `rg --files` … much faster than alternatives like `grep`."* (`codex-rs/core/gpt_5_codex_prompt.md` **[Verified]**)
- opencode ships `grep` and `glob` as first-class tools, with a description that encodes an escalation policy — `src/tool/grep.txt` **[Verified]**: *"Returns file paths and line numbers … If you need to identify/count matches, use the Bash tool with `rg` directly. Do NOT use `grep`. When you are doing an open-ended search that may require multiple rounds of globbing and grepping, use the Task tool instead."* — three tiers: structured search tool → raw ripgrep → **delegate to a subagent** so the intermediate hits burn a child's context window, not yours (Ch. 4.5).
- Cline bundles a ripgrep-based file walker with an IDE-index fallback (`apps/vscode/src/services/search/file-search.ts:18-44`: `FileSearchSource = "host_index" | "ripgrep"` **[Verified]**).
...
**Repo skeletons (SWE-agent `filemap`).** A ~40-line script installed in the sandbox — `SWE-agent/tools/filemap/bin/filemap` **[Verified]**:
...
**opencode embeds a real LSP client in a terminal agent** — the strongest version of this idea in the workspace. `src/lsp/` (client, server lifecycle, per-language launch config, diagnostics) plus a model-facing tool — `src/tool/lsp.ts` **[Verified]**:
...
An agent's context contains claims about file contents that age badly — the user edits a file mid-session, or another tool rewrites it. Cline maintains a `FileContextTracker` (`apps/vscode/src/core/context/context-tracking/FileContextTracker.ts` **[Verified]**) recording when files enter context and watching for external modification, so the runtime can warn the model that its knowledge is stale rather than let it edit against a phantom copy. opencode's edit tool enforces the same invariant transactionally: **edits require a prior read**, and the file's modification time is checked against the read (`tool/edit.ts` reads-before-writes flow **[Verified]**). This is optimistic concurrency control, with the model as the transaction that must retry.
